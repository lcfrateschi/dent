...
export const guiaTiss = pgTable(
  'guia_tiss',
  {
    ...
    numero: numeric('numero', { precision: 12, scale: 0 })
      .notNull()
      .unique()
      .default(sql`nextval('guia_numero_seq')`),
...
export const itemGuia = pgTable(
  'item_guia',
  {
    ...
    guiaId: uuid('guia_id')
      .notNull()
      .references(() => guiaTiss.id, { onDelete: 'cascade' }),
    /**
...
export const glosa = pgTable(
  ...
  {
    id: uuid('id').primaryKey().defaultRandom(),
    itemGuiaId: uuid('item_guia_id')
      .notNull()
      .references(() => itemGuia.id, { onDelete: 'restrict' }),
...
export const recursoGlosa = pgTable(
  ...
  {
    id: uuid('id').primaryKey().defaultRandom(),
    glosaId: uuid('glosa_id')
      .notNull()
      .references(() => glosa.id, { onDelete: 'restrict' }),
...
export const repasseItem = pgTable(
  'repasse_item',
  {
    ...
    repasseId: uuid('repasse_id')
      .notNull()
      .references(() => repasse.id, { onDelete: 'cascade' }),
    itemGuiaId: uuid('item_guia_id')
      .notNull()
      .references(() => itemGuia.id, { onDelete: 'restrict' }),
    valor: numeric('valor', { precision: 10, scale: 2 }).notNull(),
